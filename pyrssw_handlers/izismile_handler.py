...
IZISMILE_RSS_URL = "https://feeds2.feedburner.com/izismile"
IZISPICY_RSS_URL = "https://izispicy.com/rss.xml"


class IzismileHandler(PyRSSWRequestHandler):
    """Handler for izismile or izispicy websites.
    
    Handler name: izismile

    RSS parameters:
     - site: izispicy for izispciy, anything else for izismile


    Content:
        Get readable content of the target article content. Gather all pages into one.
    """
    ...
    def get_feed(self, parameters: dict, session: requests.Session) -> str:
        rss_url = IZISMILE_RSS_URL
        if parameters.get("site", "") == "izispicy":
            rss_url = IZISPICY_RSS_URL
        feed = session.get(url=rss_url, headers=HEADERS).text
        ...
        for spicy_link in spicy_links:
            ...
            for link in dom.xpath("//p/a[contains(@href, 'https://izispicy.com')]"):
                ...
                spicy_feeds += """<item>
                    ...
                    title,
                    link.attrib["href"],
                    etree.tostring(link.getparent().getnext(), encoding="unicode"),
    ...
    def get_content(
        ...
        urls: List[str] = [
            url,
            "%s/page,1,%s" % ("/".join(url.split("/")[:-1]), url.split("/")[-1]),
        ]
        nb_pages = 1
        ...
        while (
            url_next_page != ""
            and nb_pages <= MAX_NB_PAGES
            and url_next_page not in urls
        ):
    ...
    def _get_content(
        ...
        if len(pagers) > 1 and len(dom.xpath('//*[@class="postpages"]//a')) > 0:
            url_next_page = cast(
                str, dom.xpath('//*[@class="postpages"]//a')[-1].values()[0]
            )
        ...
        if len(comments_nodes) > 0:
            ...
            comments = cast(str, etree.tostring(comments_nodes[0], encoding="unicode"))
        ...
        if len(post_lists) > 0:
            content = etree.tostring(post_lists[0], encoding="unicode")
        else:
    ...
    def _clean_content(self, c):
        ...
        content = re.sub(
            r'<span class="sordering"><a class="back" href="#[^"]*"/><a name="[^"]*">[^<]*</a><a class="next" href="#[^"]*"/></span>',
            "",
            content,
        )
