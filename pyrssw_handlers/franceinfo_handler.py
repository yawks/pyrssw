...
class FranceInfoHandler(PyRSSWRequestHandler):
    ...
    def get_feed(self, parameters: dict, session: requests.Session) -> str:
        ...
        for link in xpath(dom, "//item/link"):
            ...
            link.getparent().xpath(".//pubDate")[0].text = re.sub(
                r"([a-z]) (\d\d)",
                r"\1 20\2",
                link.getparent().xpath(".//pubDate")[0].text,
            )
        feed = to_string(dom)
    ...
    def get_content(
        ...
        if article_intro_start_idx > -1:
            article_intro_end_idx = content[article_intro_start_idx:].find("</p>")
            content = content.replace(
                "%s</p>"
                % content[
                    article_intro_start_idx : article_intro_start_idx
                    + article_intro_end_idx
                ],
                "<h1>%s</h1>"
                % content[
                    len('<p class="article__intro">')
                    + article_intro_start_idx : article_intro_start_idx
                    + article_intro_end_idx
                ],
            )
        ...
        return PyRSSWContent(
            ...
            #franceinfo_handler h2 span.mce-nbsp-wrap {display: none;}
